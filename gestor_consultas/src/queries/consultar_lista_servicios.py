...
class ConsultarListaServicios(BaseQuery):
    ...
    # Función que realiza la consulta de un plan de alimentacion con base al id_usuario
    def query(self):
        try:
            # Logica de negocio
            response = consumir_servicio_usuarios(self.headers)
            validar_permisos_usuario(response)
            servicios = db.session.query(Servicios.id, Servicios.nombre, Servicios.costo, Servicios.lugar).filter(Servicios.estado == 'ACT').all()
            if servicios == None:
                raise NoRecordsFound
            return consulta_servicios_schema.dump(servicios)
